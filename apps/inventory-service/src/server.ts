...
import { resolvers } from './graphql/resolvers/inventory.resolvers';
import { plugins } from './middleware';
import { createContext } from './context';

const typeDefs = parse(
  readFileSync(join(__dirname, 'graphql/schema/inventory.graphql'), 'utf-8'),
);
...
async function main() {
  ...
  console.log(`inventory-service ready at ${url} (federated subgraph)`);
}
